    ...
    async fn process_job_impl(
        ...
        if let Some(blocks_proving_percentage) = config.blocks_proving_percentage {
            ...
            if threshold > blocks_proving_percentage && !shall_force_process_block {
                ...
                // TODO: will support compression later
                // transaction
    ...
    async fn get_next_job(&self) -> anyhow::Result<Option<(Self::JobId, Self::Job)>> {
            ...
            .await
        {
            Some(block_number) => {
                ...
                olaos_logs::info!(
                    "WITNESS_GENERATOR_METRICS AggregationRound::BasicCircuits blob_fetch_time {:?}",
                    started_at.elapsed()
                );
    ...
    async fn save_result(
        ...
        match optional_artifacts {
            ...
            Some(artifacts) => {
                ...
                olaos_logs::info!(
                    "BasicCircuits job_id {}, blob_save_time {:?}",
                    job_id,
...
async fn generate_witness(
    ...
    let entrypoint_code_bytes = connection
        .storage_dal()
        .get_factory_dep(header.base_system_contracts_hashes.entrypoint)
        .await
        .expect("Bootloader bytecode should exist");
    let bootloader_code = bytes_to_chunks(&entrypoint_code_bytes);
    ...
    let hashes: HashSet<H256> = input
        ...
        .filter(|&&hash| hash != h256_to_u256(header.base_system_contracts_hashes.entrypoint))
        .map(|hash| u256_to_h256(*hash))
        .collect();

    // TODO: gas not supported yet
