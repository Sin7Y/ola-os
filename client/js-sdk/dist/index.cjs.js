...
var olaCrypto = require('@sin7y/ola-crypto');
var olaAbiWasm = require('@sin7y/ola-abi-wasm');
...
function toUint64Array(value) {
    let bytes = value instanceof Uint8Array ? value : ethers.toBeArray(value);
    if (bytes.length % 8 !== 0) {
    ...
    for (let i = 0; i < chunkLength; i++) {
        const value = ethers.toBigInt(bytes.slice(i * 8, 8 * (i + 1)));
        result[i] = value;
    }
    return result;
}
...
function poseidonHash(data) {
    return Uint8Array.from(olaCrypto.poseidon_u64_bytes_for_bytes_wrapper(data));
}

function getAbiBytes(abi) {
    const abiJson = JSON.stringify(abi);
    return ethers.toUtf8Bytes(abiJson);
}
/**
 *
 * @param abi abi array
...
function encodeAbi(abi, method, params) {
    const result = olaAbiWasm.encode_input_from_js(getAbiBytes(abi), method, params);
    return BigUint64Array.from(result.map((item) => BigInt(item)));
}
function decodeAbi(abi, method, data) {
    const result = olaAbiWasm.decode_output_from_js(getAbiBytes(abi), method, data);
    return result;
}
...
async function signTransactionRequest(signer, tx) {
    ...
    sigBytes.set(ethers.toBeArray(signature.r), 0);
    sigBytes.set(ethers.toBeArray(signature.s), 32);
...
function rlp_tx(tx, signature, chain_id) {
    ...
    if (tx.type == TransactionType.EIP712Transaction ||
        ...
        if (tx.eip712_meta != null) {
            ...
            if (tx.eip712_meta.custom_signature &&
                ethers.getBytes(tx.eip712_meta.custom_signature).length == 0) {
    ...
    return ethers.ethers.concat([
        ...
        ethers.ethers.encodeRlp(fields),
    ]);
}
...
function computePublicKey(privateKey) {
    return "0x" + ethers.SigningKey.computePublicKey(privateKey).slice(4);
}
function privateKeyFromSeed(seed) {
    let privateKey = ethers.sha256(seed);
    let count = 0;
    while (count < 10000) {
...
class OlaSigner {
    constructor(privateKey) {
        ...
        const hashBytes = poseidonHash(ethers.toBeArray(this.publicKey));
        this.address = ethers.hexlify(hashBytes);
    }
    ...
    signMessage(message) {
        ...
        const hexMessage = ethers.hexlify(message);
        const privKey = new ethers.SigningKey(this.privateKey);
...
class OlaWallet {
    ...
    async call(abi, method, to, params) {
        ...
        const call_request = {
            from: this.address,
            to: to,
            data: ethers.hexlify(toUint8Array(bizCalldata)),
            nonce: ethers.toBeHex(nonce),
            transaction_type: ethers.toBeHex(TransactionType.OlaRawTransaction),
        };
    ...
    async setPubKey() {
        return this.invoke(ACCOUNT_ABI, "setPubkey(fields)", ethers.hexlify(toUint8Array(DEFAULT_ACCOUNT_ADDRESS)), [{ Fields: toBigintArray(this.signer.publicKey) }], { nonce: 0 });
    }
...
exports.OlaSigner = OlaSigner;
exports.OlaWallet = OlaWallet;
exports.createEntrypointCalldata = createEntrypointCalldata;
exports.createTransaction = createTransaction;
exports.decodeAbi = decodeAbi;
exports.encodeAbi = encodeAbi;
exports.poseidonHash = poseidonHash;
exports.toUint64Array = toUint64Array;
