...
impl OlaTree {
    ...
    pub fn reader(&self) -> OlaTreeReader {
        let db = self.tree.db.inner().clone();
        OlaTreeReader(MerkleTree::new(db))
    }
    ...
    fn process_l1_batch_full(
        ...
        let mut pre_root_hash = starting_root_hash.0;
        ...
        for (log, instruction) in output.logs.iter().zip(instructions) {
            ...
            let empty_subtree_hashes =
                (0..empty_levels_end).map(|i| PoseidonHasher.empty_subtree_hash(i));
...
/// Readonly handle to a [`OlaTree`].
#[derive(Debug)]
pub struct OlaTreeReader(MerkleTree<RocksDBWrapper>);

// While cloning `MerkleTree` is logically unsound, cloning a reader is reasonable since it is readonly.
impl Clone for OlaTreeReader {
    fn clone(&self) -> Self {
