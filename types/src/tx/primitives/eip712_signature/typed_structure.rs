...
use super::{EncodeBuilder, StructBuilder, TypeBuilder};
...
impl Eip712Domain {
    ...
    pub const NAME: &'static str = "Ola";
    /// Version of the protocol.
    pub const VERSION: &'static str = "0";
...
pub trait EIP712TypedStructure: Serialize {
    ...
    fn hash_struct(&self) -> H256 {
        // hashStruct(s : 𝕊) = hash(hash(encodeType(typeOf(s))) ‖ encodeData(s)).
        let type_hash = {
            let encode_type = self.encode_type();
            hash_bytes(encode_type.as_bytes())
        };
        let encode_data = self.encode_data();

        let mut bytes = Vec::new();
        bytes.extend_from_slice(type_hash.as_bytes().try_into().unwrap());
        for data in encode_data {
            bytes.extend_from_slice(data.as_bytes().try_into().unwrap());
        }

        hash_bytes(&bytes).into()
    }
